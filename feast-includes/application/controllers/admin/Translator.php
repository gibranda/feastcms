...
class Translator extends Admin_Controller {
		...
		if ( $this->_configured() ) {
			...
			if ( $this->validated && $this->input->post('ConfirmSaveLang') ) {
				if ( $this->_translation_saved( $result ) ) {
					$this->session->set_flashdata('message', feast_line('saved', feast_line('language')));
					redirect('admin/translator','refresh');

				} else {
					$this->data['main_content'] = 'translator/saveFailed';
					$this->data['page_title'] = 'Failure';
				}
			}
			elseif ( $this->validated &&  $this->input->post('SaveLang') ) {
				$this->data['main_content'] = 'translator/saveLang';
				$this->data['page_title'] = 'Confirm';
			}
			else {
				$this->data['main_content'] = 'translator/translateLang';
				$this->data['page_title'] = 'Translate ' . $this->langModule;
			}

		}

        $this->load->ext_view('admin', 'layouts/main',$this->data);
    ...
    public function newlang()
    {
    	...
    	if(is_demo() == FALSE) {
	        ...
	        $path = $this->langDir . '/' . $lang;

	        if(!is_dir($path)) //create the folder if it's not already exists
	        {
	          if(mkdir($path,0755,TRUE)) {
	              $content = "<?php  if (!defined('BASEPATH')) exit('No direct script access allowed');";
	              $fp = fopen($this->langDir . '/' . $lang . '/file_lang.php',"wb");
	              fwrite($fp,$content);
	              fclose($fp);
	                $this->slaveLang = $lang;
	                $this->session->set_flashdata('message', feast_line('saved', feast_line('language')));
			redirect('admin/translator');
	                ...
	                $this->session->set_flashdata('error', 'Language can not be created');          	
	          }
    ...
    public function delete($lang)
    {
    	...
    	if(is_demo() == FALSE) {
	        if(is_dir( $this->langDir . '/' . $lang )) {
	          if(unlink($this->langDir . '/' . $lang . '/file_lang.php')) {
	              if(rmdir( $this->langDir . '/' . $lang )) {
	                $title = 'title_'.$lang;
	                $body = 'body_'.$lang;
	                $this->db->query('ALTER TABLE post_meta DROP COLUMN '.$title);
	                $this->db->query('ALTER TABLE post_meta DROP COLUMN '.$body);
	                $this->session->set_flashdata('message', feast_line('deleted', feast_line('language')));

	                redirect('admin/translator');
	              } else {
	                $this->session->set_flashdata('error', 'Language can not be deleted');
	                redirect('admin/translator');
	              }
	          } else {
	                $this->session->set_flashdata('error', 'Language can not be deleted');
	...
	function _configured() {
		...
		elseif ( ! $this->input->post('slaveLang') ) {
			...
			$this->data['main_content'] = 'translator/selectSlaveLang';
		...
		elseif ( ! $this->langModule ) {
			...
			$this->data['main_content'] = 'translator/selectModule';
